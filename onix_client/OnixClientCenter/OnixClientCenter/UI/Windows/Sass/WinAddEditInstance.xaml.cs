...
{
    public partial class WinAddEditInstance : Window
    {
        ...
        {
            if (!CHelper.VerifyAccessRight("SASS_VM_EDIT"))
            {
                return (false);
            }
            ...
            if (Mode.Equals("A"))
            {
                if (SaveToView())
                {
                    ...
                    CTable newobj = OnixWebServiceAPI.SubmitObjectAPI("SassCreateInstance", vw.GetDbObject());
            ...
            else if (Mode.Equals("E"))
            {
                if (vw.IsModified)
                {
                    ...
                    {
                        CUtil.EnableForm(false, this);
                        CTable t = OnixWebServiceAPI.SubmitObjectAPI("SassUpdateInstance", vw.GetDbObject());
                        CUtil.EnableForm(true, this);
                        if (t != null)
                        {
        ...
        private void LoadData()
        {
            ...
            if (Mode.Equals("E"))
            {
                CTable m = OnixWebServiceAPI.SubmitObjectAPI("SassGetInstanceInfo", actualView.GetDbObject());
                if (m != null)
                {
                    vw.SetDbObject(m);
