...
{
    public partial class WinAddEditAccountMiscDocItem : Window
    {
        ...
        private Boolean isForRevenue = false;
        ...
        public WinAddEditAccountMiscDocItem(Boolean isRevenue)
        {
            isForRevenue = isRevenue;
            InitializeComponent();
        }
        ...
        private LookupSearchType2 selectionTypeToLookup(String selType)
        {
            ...
            if (isForRevenue)
            {
                lkup = LookupSearchType2.ServiceOtherSaleLookup;
            }
            else
            {
                lkup = LookupSearchType2.ServiceOtherPurchaseLookup;
            }
        ...
        private void LoadData()
        {
            ...
            if (Mode.Equals("E"))
            {
                ...
                lkup.Lookup = selectionTypeToLookup(vw.SelectTypeMisc);

                vw.NotifyAllPropertiesChanged();
            }
            else
            {
                MAccountDocItem m = getLatestItem(parentView.AccountItem);
                if (m != null)
                {
                    vw.SelectTypeMisc = m.SelectTypeMisc;
                    //CTable temp = m.TempObj.GetDbObject().CloneAll();
                    vw.TempObj = m.TempObj;
                    vw.NotifyAllPropertiesChanged();
                }
                else
                {
                    vw.SelectTypeMisc = "1"; //CGlobalVariable.GetGlobalVariableValue("DEFAULT_SELECTION_TYPE_PERCHASE");
                }
